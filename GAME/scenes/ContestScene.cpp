...
ContestScene::ContestScene(kEngine *system) {
  ...
  // 観客用椅子（中央）
  audienceChairsMidModelHandle_ = system_->SetModelObj(
      "GAME/resources/ContestStageObject/audienceChairMid.obj");
  ...
  // 観客用椅子（右側）
  audienceChairsSideModelHandle_ = system_->SetModelObj(
      "GAME/resources/ContestStageObject/audienceChairSide.obj");
  audienceChairsRightSide_.resize(9);
  audienceChairsLeftSide_.resize(9);
  for (int i = 0; i < 9; ++i) {
    float zPos = -2.65f - (i * 0.4f);
    SetupSceneObject(audienceChairsRightSide_[i],
                     audienceChairsSideModelHandle_, {2.0f, 0.0f, zPos},
                     {0.0f, PI - 0.35f, 0.0f}, objectScale); // 右側
    SetupSceneObject(audienceChairsLeftSide_[i], audienceChairsSideModelHandle_,
                     {-2.0f, 0.0f, zPos}, {0.0f, PI + 0.35f, 0.0f},
                     objectScale); // 左側
  }
