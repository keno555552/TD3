...
void TravelScene::Update() {
  ...
  if (logFrame % 10 == 0) {
    // Logger::Log("---- LAND / PUSH DEBUG ----");
    // Logger::Log("isGrounded: %d", isGrounded_ ? 1 : 0);
    // Logger::Log("wasGrounded: %d", wasGrounded ? 1 : 0);
    // Logger::Log("justLanded: %d", (!wasGrounded && isGrounded_) ? 1 : 0);

    // Logger::Log("leftNowInput: %d", leftNowInput ? 1 : 0);
    // Logger::Log("rightNowInput: %d", rightNowInput ? 1 : 0);

    // Logger::Log("velocityY: %.5f", velocityY_);

    // Logger::Log("leftHoldTime: %.5f", leftHoldTime_);
    // Logger::Log("rightHoldTime: %.5f", rightHoldTime_);
